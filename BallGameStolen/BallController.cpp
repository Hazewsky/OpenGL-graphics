...
void BallController::updateBalls(std::vector <Ball>& balls, Grid* grid, float deltaTime, int maxX, int maxY) {  
    ...
    for (size_t i = 0; i < balls.size(); i++) {
		...
		//Проверка, двинулся ли шар
		Cell* newCell = grid->getCell(ball.position);
		if (newCell != ball.ownerCell){
			//Надо сдвинуть шар
			//Удаляем из вектора где в данный момент и впихиваем в вектор клетки куда идет
			//и меняем указатель
	...
	//обновление используя spatial partitining
...
//	}
	...
	//после коллизии, ибо тоже может изменить позицию
}
		...
		for (int j = 0; j < cell.balls.size(); j++){
			...
			//	//bottom left
				if (y < grid->m_numYCells - 1){
					checkCollision(ball, grid->getCell(x - 1, y + 1)->balls/*to left*/, 0);
				}
			}
			////up cell
			if (y > 0){
				checkCollision(ball, grid->getCell(x, y - 1)->balls/*to left*/, 0);
			}
		}
	}
}
    ...
    if (collisionDepth > 0) {
      ...
      //  float massRatio = b1.mass / b2.mass;
		...
		if (glm::length(b1.velocity + b2.velocity) > 0.5f){
			//choosethe faster ball
			bool choice = glm::length(b1.velocity) < glm::length(b2.velocity);

			//faste ball transfers its color to the slower
